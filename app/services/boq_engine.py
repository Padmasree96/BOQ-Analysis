"""
boq_engine.py — CAD BOQ Consolidation Engine

Adapted from backend/boq_engine.py for the BOQ-Analysis app.
Rate assignment is removed since this app uses pure Quantity Takeoff (no pricing).

KEY FLOW:
  1. Takes raw items from cad_parser (no dedup, 476+ items)
  2. Groups items by clean_name + category
  3. Sums quantities for same item type
  4. Returns consolidated BOQ line items
"""
...
def consolidate_items(raw_items: list[dict]) -> list[dict]:
    """
    Take the full raw list (476 items, possibly duplicated across layouts)
    ...
    for item in raw_items:
        ...
        if key not in groups:
            groups[key] = {
                ...
                "count":       1,   # how many raw instances found
            }
        else:
            existing = groups[key]
            # Sum quantities
    ...
    for idx, (key, item) in enumerate(groups.items(), 1):
        ...
        if item["quantity"] == 0 and item["count"] > 1:
            # For countable items, instance count IS the quantity
            if unit in ("nos","sets"):
...
def apply_layer_geometry(boq_items: list[dict], layer_summary: dict) -> list[dict]:
    ...
    for lname, ldata in layer_summary.items():
        ...
        if llen < 5: continue # Skip tiny lengths
        
        items = layer_to_items.get(lname, [])
        # Only apply if we have exactly one linear item (to avoid double counting)
...
def generate_boq(raw: dict) -> list[dict]:
    ...
    # Add geometry items
    items_map = [
        ("Conduit/Cable Runs","Electrical",raw.get("total_line_length",0),"m"),
    ...
    for name, cat, qty, unit in items_map:
        if qty and qty > 0:
            consolidated.append({
