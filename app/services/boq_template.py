"""
boq_template.py — Fallback mapping.
"""
...
_SKIP_FRAGMENTS = re.compile(
    r"^(for\s+lighting\s*[&+]\s*power|each\s+solar\s+panel\s+generates"
    r"|around\s+[\d.]+\s*kw\s+of\s+solar|along\s+with\s+other\s+switch\s+controls"
    r"|from\s+metering\s+panel|fed\s+from\s+common\s+area\s+db"
    r"|ep\s+for\s+panel|conduit\s+legends|socket\s+outlet|to\s+cable\s+chamber"
    ...
    r"|xql;|xqc;|xt[\d\.]|@\s*\d+mm\s*(aff|ffl|abc))",
    re.IGNORECASE,
)

_CAT_KW = {
    "Mechanical":  ["vrv unit","vrf unit","ahu","fcu","solar panel","bulkhead light","exhaust fan"],
...
def map_to_boq(raw: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for b in raw.get("blocks_with_attribs", []):
        ...
        _add(name, b.get("description",name), 1, "nos", b.get("category") or _classify(name))

    for t in raw.get("texts", []):
        name = t.get("clean_name") or t.get("text","")
        _add(name, t.get("text",name), t.get("quantity",0), t.get("unit","nos"), t.get("category") or _classify(name))

    if not items:
        tl = raw.get("total_line_length",0)
        if tl > 0: _add("Linear runs", "Total line length", tl, "m", "General")
        for d,c,n in [("door_count","Architecture","Doors"), ("window_count","Architecture","Windows"), ("column_count","Structural","Columns")]:
            v = raw.get(d,0)
            if v: _add(n, f"{n} from blocks", v, "nos", c)
    return items
